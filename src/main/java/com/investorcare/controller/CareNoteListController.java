 ...
 * @author quyt2
 */
public class CareNoteListController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = "careNoteList.jsp";
        HttpSession session = request.getSession();
        try {
            CareNoteDAO dao = new CareNoteDAO();
            User loginUser = (User) session.getAttribute("LOGIN_USER");
            if (loginUser != null && "User".equalsIgnoreCase(loginUser.getRole())) {
                List<CareNote> listCN = dao.getCareNoteByUserId(loginUser.getUserId());
                request.setAttribute("LIST_CARENOTE", listCN);
            } else {
                url = "login.jsp";
            }
